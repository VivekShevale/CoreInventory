    ...
    >
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-3">
          ...
          <h3 className="font-medium text-zinc-900 dark:text-white text-base">
            {title}
          </h3>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {items.map((item, i) => (
          <div
            key={i}
            className={`rounded-lg ${c.light} border ${c.border} p-3 text-center`}
          >
...
export default function DashboardPage() {
  ...
  // ── KEY FIX ───────────────────────────────────────────────────────────────
  // The API returns { ..., charts: { kpis, movements, ... } }
  // Redux stores the full response in stats.
  // We pass stats.charts to DashboardCharts.
  // When stats is null (first render), chartStats is undefined →
  // DashboardCharts shows skeletons, not demo data.
  const chartStats = s.charts ?? null;
  // ─────────────────────────────────────────────────────────────────────────
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-4 text-xs text-zinc-500 dark:text-zinc-400 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg p-4">
        <span className="flex items-center gap-1.5">
          ...
          <span>
            <b className="text-zinc-700 dark:text-zinc-300">Late</b> = scheduled
            date &lt; today
          </span>
        </span>
        <span className="flex items-center gap-1.5">
          ...
          <span>
            <b className="text-zinc-700 dark:text-zinc-300">Scheduled</b> =
        ...
        <span className="flex items-center gap-1.5">
          ...
          <span>
            <b className="text-zinc-700 dark:text-zinc-300">Waiting</b> =
      ...
      <div className="flex items-center gap-3 pt-2">
        ...
        <span className="text-xs font-semibold text-zinc-400 dark:text-zinc-500 uppercase tracking-widest px-2">
          Analytics
        </span>
        <div className="h-px flex-1 bg-zinc-200 dark:bg-zinc-800" />
      </div>
      ...
      {/*
        DashboardCharts receives stats.charts from the API response.
        - null  → shows animated skeletons (loading state)
        - object → renders all charts with real data
        No demo data fallback — charts only show what the DB actually has.
      */}
