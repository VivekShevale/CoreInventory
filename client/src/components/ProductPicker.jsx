...
export default function ProductPicker({ open, onClose, lines, onChange }) {
  ...
  useEffect(() => {
    if (!open) return;
    setLoading(true);
    api.get('/api/products/', { params: { search } })
  ...
  const addProduct = (product) => {
    ...
    onChange([...lines, { product_id: product.id, product_name: product.name, product_sku: product.sku, quantity: 1, on_hand: product.on_hand }]);
  };
  ...
  return (
    <Modal open={open} onClose={onClose} title="Add Products" size="lg">
      <div className="flex flex-col gap-4">
        ...
        {lines.length > 0 && (
          <div>
            ...
            <div className="flex flex-col gap-2">
              {lines.map(line => (
                <div key={line.product_id} className="flex items-center gap-3 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl px-3 py-2.5">
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-slate-800 dark:text-white truncate">
                      [{line.product_sku}] {line.product_name}
                    </p>
                    {line.on_hand !== undefined && (
                      <p className="text-xs text-slate-400">On hand: {line.on_hand}</p>
                    )}
                  </div>
                  <div className="flex items-center gap-1.5">
                    ...
                    <input
                      ...
                      className="w-16 text-center text-sm font-semibold border border-slate-200 dark:border-slate-700 rounded-lg py-1 bg-white dark:bg-slate-800 text-slate-800 dark:text-white focus:outline-none focus:border-indigo-500"
                    />
                    <button onClick={() => updateQty(line.product_id, line.quantity + 1)} className="w-7 h-7 rounded-lg bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 flex items-center justify-center hover:bg-slate-50 transition-colors">
                      <Plus size={12} />
                    </button>
        ...
        {/* Product list */}
        <div>
          <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-2">All Products</p>
          {loading ? (
                ...
                return (
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-slate-400">On hand: {product.on_hand} {product.unit_of_measure}</p>
                    </div>
                    {!already && (
